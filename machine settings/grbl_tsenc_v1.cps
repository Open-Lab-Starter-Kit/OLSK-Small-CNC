...
function onSection() {
  ...
  if (workOffset == 0) {
    //warningOnce(localize("Work offset has not been specified. Using G54 as WCS."), WARNING_WORK_OFFSET); //DANIELE
    workOffset = 1;
  }
  if (workOffset > 0) {
    if (workOffset > 6) {
  ...
  if (insertToolCall || retracted) {
    ...
    if (!machineConfiguration.isHeadConfiguration()) {
        writeBlock(gMotionModal.format(0), zOutput.format(initialPosition.z)); //DANIELE
      writeBlock(
